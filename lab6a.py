...
class Student:
    ...
    def displayGPA(self):
        ...
        if len(self.courses) !=0:
            ...
            zero = 0.0
            return 'GPA of student ' + self.name + ' is ' + str(zero)

    # Return a list of course that the student passed (not a 0.0 grade)
    def displayCourses(self):
        course_pass = [] # in list
        for subject in self.courses.keys(): # take key values from the courses dict
            if self.courses[subject] > 0.0: # append to course_pass list when each course item is more than 0.0
                course_pass.append(subject)
        return course_pass
